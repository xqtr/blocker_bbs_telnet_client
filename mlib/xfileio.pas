...
Function  CreateFile      (FN:String):Boolean;
...
Begin
  ...
  While DosError = 0 Do Begin
    If Upper(Exempt) <> Upper(DirInfo.Name) Then
...
Var
  Count : Byte;
Begin
  If Pos(PathChar,Str)<=0 Then Begin
    Result:='';
    Exit;
  End;
...
Begin
  ...
  While DosError = 0 Do Begin
    If Upper(Dir.Name) = Upper(JustFile(FN)) Then Begin
...
Function AppName: String;
Begin
     AppName := JustFileName(ParamStr(0));
End;

Function AppPath: String;
Begin
     AppPath := AddSlash(JustPath(ParamStr(0)));
End;
...
begin
  ...
  repeat
    v:=justfilename(s)+int2str(i)+'.'+JustFileExt(s);
    i:=i+1;
  until (fileexist(v)=false);
  result:=v;
end;
