...
Begin
  If Attr = Screen.TextAttr Then Begin
    AttrToAnsi := '';
    Exit;
  End;
...
Begin
  ...
  GetCharAt := Screen.getcharat(ax,ay);
End;
...
Procedure RestoreScreen(screenBuf: TConsoleImageRec);
Begin
  Screen.RestoreScreen(Screenbuf);
End;

Procedure SaveScreen(var screenBuf: TConsoleImageRec);
Begin
  Screen.SaveScreen (screenBuf);
End;

Procedure SetAttrAt(AAttr, AX, AY: Byte);
Begin
  screen.setattrat(aattr,ax,ay);
End;


Procedure SetCharAt(ACh: Char; AX, AY: Byte);
Begin
  screen.setcharat(ach,ax,ay);
End;
...
Procedure WritePipe (Text: String);
begin
  writexypipe(wherex,wherey,screen.textattr,text);
end;
...
Function KeyPressed : Boolean;
Begin
  Result:=screen.KeyPressed
End;

Function ReadKey : Char;
Begin
  Result:=screen.ReadKey
  //Result:=Crt.ReadKey;
End;
