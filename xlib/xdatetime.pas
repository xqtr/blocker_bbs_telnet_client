...
function  uint2str            (i : longint; l : integer) : string_10;
function  ustr2int            (s : string) : longint;
...
Begin
  ...
  Y := Copy(Int2Str(Trunc(Temp3)), 3, 2);
...
Var
  ...
  Temp2 : Real;
Begin
  Month := Str2Int(Copy(Str, 1, 2));
  Day   := Str2Int(Copy(Str, 4, 2));
  Year  := Str2Int(Copy(Str, 7, 2));
...
Begin
  ...
  DT.Month := Str2Int(Copy(Str, 1, 2));
  DT.Day   := Str2Int(Copy(Str, 4, 2));
  DT.Hour  := 0;
  DT.Min   := 0;
  DT.Sec   := 0;
...
Var
  ...
  D : Byte;
Begin
  M := Str2Int(Copy(Str, 1, 2));
  D := Str2Int(Copy(Str, 4, 2));
...
Begin
  Result  := Mask;
  YearStr := Int2Str(DT.Year);
  Result  := Replace(Result, 'YYYY', YearStr);
  Result  := Replace(Result, 'YY', Copy(YearStr, 3, 2));
  Result  := Replace(Result, 'MM', strZero(DT.Month));
  Result  := Replace(Result, 'DD', strZero(DT.Day));
  Result  := Replace(Result, 'HH', strZero(DT.Hour));
  Result  := Replace(Result, 'II', strZero(DT.Min));
  Result  := Replace(Result, 'SS', strZero(DT.Sec));
  Result  := Replace(Result, 'NNN', MonthString[DT.Month]);
End;
...
Begin
     ...
     M := ASec div 60;
     S := ASec mod 60;
     SecToDHMS := Int2Str(D) + 'd ' + Int2Str(H) + 'h ' + Int2Str(M) + 'm ' + Int2Str(S) + 's';
End;
...
Begin
     ...
     ASec := ASec mod 3600;
     M := ASec div 60;
     SecToHM := xstrings.PadLeft(Int2Str(H), '0', 2) + ':' + xstrings.PadLeft(Int2Str(M), '0', 2);
End;
...
Begin
     ...
     M := ASec div 60;
     S := ASec mod 60;
     SecToHMS := xstrings.PadLeft(Int2Str(H), '0', 2) + ':' + xstrings.PadLeft(Int2Str(M), '0', 2) + ':' + xstrings.PadLeft(Int2Str(S), '0', 2);
End;
...
Begin
     M := ASec div 60;
     S := ASec mod 60;
     SecToMS := xstrings.PadLeft(Int2Str(M), '0', 2) + ':' + xstrings.PadLeft(Int2Str(S), '0', 2);
End;
...
begin
   UnpackUnixTime(t, dt);
   FormatUnixDate :=
      StrPadL(uint2str(dt.day, 2),2,'0')          +'-'+
      StrPadL(uint2str(dt.month,2   ),2,'0')      +'-'+
      uint2str(dt.year, 4);
end;
